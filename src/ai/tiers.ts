...
// Sirius's topK is tuned down from SPEC.md §3.3's initial value of 24 to 14 — see DECISIONS.md.
// Search cost per depth level scales with topK^depth, so a large topK (vs Rigel's 16) consumes
// Sirius's larger time budget on extra breadth rather than the extra depth that's supposed to
// be its distinguishing advantage over Rigel. Deliberately narrower than Rigel's own topK so
// Sirius's bigger time budget reliably buys it more depth, not just parity.
export const TIERS: Readonly<Record<TierName, TierConfig>> = {
  ...
  Sirius: { name: 'Sirius', depth: 30, topK: 14, noise: 0, timeBudgetMs: 2500 },
};
